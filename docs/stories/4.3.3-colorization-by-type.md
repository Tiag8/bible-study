...
**Quero** ver cores diferentes para tipos de referência (interna/externa/bidirecional)
**Para** identificar rapidamente o contexto e tipo de cada referência

### Why This Matters
- 👁️ **Clareza Visual**: Cores naturalmente indicam tipo/direção
- ⚡ **UX Rápida**: Identifica tipo sem ler texto
- 🎨 **Padrão Obsidian**: Alinha com design de second-brain apps

---

## ✅ CRITÉRIOS DE ACEITAÇÃO

### A. Cores Semânticas Aplicadas

```gherkin
Scenario: Referência que EU criei (A→B)
  Given estou em Salmos 10
  And criei referência para Provérbios 21
  When ReferencesSidebar renderiza
  Then vejo card COM FUNDO VERDE CLARO
  And contraste do texto é WCAG AA
  And ícone opcional (ou apenas cor diferencia)

Scenario: Referência criada para MIM (B←A)
  Given estou em Provérbios 21
  And Salmos 10 referencia este estudo
  When ReferencesSidebar renderiza
  Then vejo card COM FUNDO VERMELHO CLARO
  And é visualmente diferente do verde
...
- [ ] Cores são claras (não saturadas)
...
Scenario: Cor mantida em hover
  Given vejo card referência em verde
  When passo mouse
  Then permanece verde (com hover effect)
  And ações aparecem (delete, reorder buttons)

Scenario: Cor mantida em drag
  Given estou arrastando referência verde
  When levanto mouse
  Then card não perde cor
  And opacity pode mudar (fade) mas cor mantém

Scenario: Cor mantida em loading
  Given ReferencesSidebar está carregando
  When skeleton renderiza
  Then skeleton tem cor correta (verde/vermelho/azul)
  And loading animator não apaga cores
```

**Validações Técnicas**:
- [ ] Hover: CSS `hover:bg-green-100` mantém cor
- [ ] Drag: opacity 0.5 mas cor preservada
- [ ] Loading skeleton: mesmo cor, só sem conteúdo
...
Scenario: Usar src/lib/design-tokens.ts
  ...
  Then código é limpo e centralizador
  And mudanças de cor são 1 lugar só

Scenario: Fallback se token não existir
  Given REFERENCE_TYPE_COLORS não existe ainda
...
### D. Sem Ícones/Badges (Apenas Cor)

```gherkin
Scenario: Diferenciação é APENAS pela cor
  Given não adicionar ícones→, ←, 🔗
  And não adicionar badges "Referencia", "Referenciado"
  When card renderiza
  Then cor é único diferenciador
...
- [ ] **Adicionar novo export**
  ...
  export const REFERENCE_TYPE_COLORS = {
    // "Eu referencio" (A→B, is_bidirectional: true)
    references: 'bg-green-50 border-green-200 hover:bg-green-100',

    // "Fui referenciado" (B←A, is_bidirectional: false)
...
- [ ] **Criar helper para determinar cor**
  ...
  export function getReferenceTypeLabel(reference: Reference): string {
    ...
    if (reference.is_bidirectional === true) return 'Referência';
...
- [ ] **Card já renderiza, apenas aplicar classe**
  ```typescript
  // Se ReferenceCard é used elsewhere, aplicar cor também
...
- [ ] **Skeleton carrega com cor correta**
  ...
  // Simplificação: skeleton genérico (sem cores diferentes)
  // Ou: skeleton com cor padrão (azul/gray)
  ```
...
| Component updates (className aplicação) | 0.5h | 1 |
...
- [ ] **Verde = Referência (A→B)**
  - [ ] Criar referência
  - [ ] Verificar card em verde claro
  - [ ] Hover em verde mais escuro
  - [ ] Drag → verde com opacity 0.5

- [ ] **Vermelho = Referenciado (B←A)**
...
- [ ] **Estados**
  ...
  - [ ] Skeleton: cor padrão ou ausente
...
test('referência tem cor verde', async ({ page }) => {
  // 1. Create reference
  // 2. Verify element has bg-green-50 class
});

test('referência reversa tem cor vermelha', async ({ page }) => {
...
test('cores são preservadas em hover', async ({ page }) => {
...
- [ ] **Accessibility**: WCAG AA validado? Não apenas cor?
- [ ] **Consistency**: Cores aplicadas em TODOS componentes?
- [ ] **Performance**: Nenhuma recompilation desnecessária?

**Expected Issues**:
- 🟢 LOW: CSS class redundância (BG + hover) → Nice-to-have refactor

---

## 📋 ACCEPTANCE CHECKLIST (Para QA/PM)

- [ ] Verde aparece em referência A→B
- [ ] Vermelho aparece em referência B←A
...
### Pre-requisites
1. ✅ Story 4.3.1 e 4.3.2 merge completo (schema + hooks)
2. ✅ `Reference` type com `link_type` + `is_bidirectional`
3. ✅ Branch: `feature/4.3.3-colorization`
...
### Blockers
- 🔴 Story 4.3.1 (tipos: is_bidirectional)
- 🔴 Story 4.3.2 (tipos: link_type)

### Dependents
- Nenhuma (colorização é visualmente independente)
...
- **Color Psychology**: Verde (ação), Vermelho (reação), Azul (externo)
...
- Cores devem ser claras (50, não 600)
- Usar Tailwind defaults ou custom tokens
- Validar contraste automaticamente com axe-core
- Não adicionar ícones/badges (apenas cor diferencia)
- Helper function torna fácil mudar cores depois (1 lugar)
