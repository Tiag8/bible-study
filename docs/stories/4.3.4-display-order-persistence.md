...
**Quero** reordenar minhas referências por importância/contexto
**Para** que a ordem seja salva e restaurada quando eu voltar

### Why This Matters
- 📊 **Organização**: Permite priorizar referencias por relevância
- 💾 **Persistência**: Ordem customizada é salva permanentemente
- ⚡ **Eficiência**: Rápido acessar références mais importantes

---

## ✅ CRITÉRIOS DE ACEITAÇÃO

### A. Reordenar Referências via Drag-and-Drop

```gherkin
Scenario: Arrastar referência para cima
  Given tenho 3 referências: [A, B, C]
  And B está em posição 2
  When clico e arrasto B para posição 1
  Then vejo animação suave
  And ordem local muda: [B, A, C]
  And toast: "Ordem salva"

Scenario: Arrastar para baixo
  Given ordem é [A, B, C]
  And A está em posição 1
  When clico e arrasto A para posição 3
  Then ordem muda: [B, C, A]
  And BD atualiza `display_order`

Scenario: Ordem persiste após reload
  Given reorderei referencias em [B, A, C]
  When recarrego página
  Then ordem restaurada é [B, A, C]
  And não volta para [A, B, C]
```

**Validações Técnicas**:
- [ ] Cada referência tem `display_order` campo (0, 1, 2, ...)
- [ ] Reorder atualiza `display_order` em DB
- [ ] Refetch carrega com ordem correta
- [ ] Não há gaps (ex: [0, 2, 5] é inválido)

---

### B. Validação de Ordem

```gherkin
Scenario: Ordem é contígua (sem gaps)
  Given reordeno items
  When salvo no DB
  Then sequência é [0, 1, 2, 3, ...]
  And não há [0, 2, 4] (gaps)

Scenario: Insert nova referência
...
```gherkin
Scenario: Reorder é rápido
  Given tenho 20 referências
  When reordeno 1
  Then salva em < 100ms
...
**Validações Técnicas**:
- [ ] Query rápida (índice em source_study_id)
- [ ] Sem N+1 queries
- [ ] Toast otimista (não aguarda resposta)
...
- [ ] **Migration**: Adicionar coluna `display_order` (já em 4.3.1, validar)
  ```sql
...
- [ ] **Validação**: Trigger ou CHECK (opcional para MVP)
  ```sql
...
- [ ] **Atualizar `reorderReference()`**
  ...
  async function reorderReference(
    ...
    // 1. Encontra referência atual
    const currentRef = references.find(r => r.id === referenceId);
    if (!currentRef) throw new Error('Referência não encontrada');
    ...
    // 5. Refetch para garantir sincronização
    await refetch();
...
- [ ] **Refetch carrega com ordem correta**
  ...
  async function fetchReferences() {
    const { data, error } = await supabase
      ...
      .order('display_order', { ascending: true })  // ← KEY
...
- [ ] **Toast otimista de reorder**
  ...
  const handleReorder = async (refId: string, direction: 'up' | 'down') => {
    ...
    if (success) {
      ...
      toast.info('Já está no topo/fim');
    }
  };
  ```
...
### Frontend/Component (`DndContext` - Já Existe)

- [ ] **Validar que `@dnd-kit` persiste ordem**
  ```typescript
  // SortableContext já renderiza com ordem do array
  // Se array tiver display_order sequencial, drag-drop funciona
  // Importante: refetch após drag para garantir sincronização
  ```
...
- [ ] **Boundary conditions**
  - [ ] Clica up em item topo → nada muda
  - [ ] Clica down em item fim → nada muda
  - [ ] Toast: "Já está no topo/fim"

- [ ] **Persistência**
  - [ ] Reordena: [A, B, C] → [C, A, B]
  - [ ] Recarrega página
...
test('boundary conditions', async ({ page }) => {
  // 1. Try to move top item up → no change
  // 2. Try to move bottom item down → no change
});
```
...
**Focus Areas**:
- [ ] **Performance**: Índice em `(source_study_id, display_order)`?
- [ ] **Atomicity**: Swap é transacional? Sem race conditions?
- [ ] **UX**: Toast timing correto? Não fica preso?
- [ ] **Tests**: E2E covers reorder + persist?

**Expected Issues**:
- 🟢 LOW: Toast optimization (otimista vs real)
...
### Pre-requisites
1. ✅ Stories 4.3.1, 4.3.2, 4.3.3 merge completo
2. ✅ `display_order` coluna existe (via 4.3.1 migration)
3. ✅ Branch: `feature/4.3.4-display-order`
...
### Blockers
- 🔴 Story 4.3.1 (schema + migration base)

### Dependents
- Nenhuma (último da série)
...
- **Padrão**: dnd-kit já gerencia reordenação UI
...
- `display_order` é SMALLINT (0-32767, suficiente para refs)
- Backfill é crítica (ordena por `created_at` se display_order vazio)
- RPC swap é simples (2 UPDATEs na mesma transação)
- Refetch garante sincronização (não confiar apenas em otimista)
- Índice em `(source_study_id, display_order)` é essencial
